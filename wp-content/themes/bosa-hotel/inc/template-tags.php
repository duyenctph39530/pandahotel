...
if ( ! function_exists( 'bosa_hotel_entry_header' ) ) :
	/**
	 * Prints HTML with meta information for the categories.
	 */
	function bosa_hotel_entry_header() {
...
if ( ! function_exists( 'bosa_hotel_entry_footer' ) ) :
	/**
	 * Prints HTML with meta information for the tags and comments.
	 */
	function bosa_hotel_entry_footer() {
			...
			if( !is_single() && !get_theme_mod( 'hide_comment', false ) ){ 
				...
				comments_popup_link(
					sprintf(
						wp_kses(
							/* translators: %s: post title */
							__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'bosa-hotel' ),
							array(
								'span' => array(
									'class' => array(),
								),
		...
		if ( 'post' === get_post_type() ) {
				/* translators: used between list items, there is a space after the comma */
				$categories_list = get_the_category_list( esc_html__( ', ', 'bosa-hotel' ) );
			...
			if( get_the_tag_list() ): ?>
				<div class="tag-links">
					<span class="screen-reader-text">
						<?php echo esc_html__( 'Tags', 'bosa-hotel' ); ?>
					</span>
					<?php echo get_the_tag_list( '', esc_html__( ', ', 'bosa-hotel' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				</div>
			<?php endif; 
		} 
	}
...
if( !function_exists( 'bosa_hotel_get_day_link' ) ):
/**
...
* @return url
*/
function bosa_hotel_get_day_link(){
 ...
 * @return string
 */
if ( ! function_exists( 'bosa_hotel_time_link' ) ) :

function bosa_hotel_time_link() {
	...
	?>
	<span class="screen-reader-text"><?php echo esc_html__( 'Posted on', 'bosa-hotel' ); ?></span>
	<span class="posted-on">
		<a href="<?php echo esc_url( bosa_hotel_get_day_link() ); ?>" rel="bookmark">
...
if ( ! function_exists( 'bosa_hotel_edit_link' ) ) :

function bosa_hotel_edit_link( $echo = true ) {
	...
	edit_post_link(
		sprintf(
			...
			__( 'EDIT<span class="screen-reader-text"> "%s"</span>', 'bosa-hotel' ),
			get_the_title()
		),
