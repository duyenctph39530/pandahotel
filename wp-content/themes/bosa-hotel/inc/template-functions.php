 ...
 * @return array
 */
function bosa_hotel_body_classes( $classes ) {
	...
	if ( ( is_home() || ( is_archive() && !bosa_hotel_wooCom_is_shop() ) ) && get_theme_mod( 'disable_sidebar_blog_page', false ) ){
		$classes[] = 'no-sidebar';
	}
	...
	if ( bosa_hotel_wooCom_is_shop() && get_theme_mod( 'disable_sidebar_woocommerce_page', false ) ){
		$classes[] = 'no-sidebar';
	}
...
add_filter( 'body_class', 'bosa_hotel_body_classes' );

if( !function_exists( 'bosa_hotel_get_icon_by_post_format' ) ):
/**
...
*/
function bosa_hotel_get_icon_by_post_format(){
    $icons = array(
    ...
    return apply_filters( 'bosa_hotel_post_format_icon', $icons[ $format ] );
}
endif;
 ...
 */
function bosa_hotel_page_title_display() {
	...
	elseif ( is_search() ) : ?>
		...
		printf( esc_html__( 'Search Results for: %s', 'bosa-hotel' ), get_search_query() ); ?></h1>
	<?php elseif ( is_404() ) :
		echo '<h1 class="page-title entry-title">' . esc_html__( 'Oops! That page can&#39;t be found.', 'bosa-hotel' ) . '</h1>';
	endif;
}
 ...
 * Display page title
 */
function bosa_hotel_page_title() {
 ...
 */
function bosa_hotel_single_page_title() {
 ...
 */
function bosa_hotel_blog_page_title() {
 ...
 */
function bosa_hotel_image_size( $image_size ){
...
* @return string
*/
function bosa_hotel_modify_search_form( $form ){
	return str_replace( '</form>', '<button type="submit" class="search-button"><span class="fas fa-search"></span></button></form>', $form );
}
add_filter( 'get_search_form', 'bosa_hotel_modify_search_form' );

/**
 * Add breadcrumb
 */

if ( ! function_exists( 'bosa_hotel_breadcrumb' ) ) :

	function bosa_hotel_breadcrumb() {
...
if ( ! function_exists( 'bosa_hotel_breadcrumb_wrap' ) ) :
	/**
	...
	function bosa_hotel_breadcrumb_wrap( $transparent = false ) {
		if( !is_home() ) { ?>
			<div class="breadcrumb-wrap">
	        	<?php if( $transparent ){ ?>
	        		<div class="container">
	        			<?php bosa_hotel_breadcrumb(); ?>
	        		</div>
				<?php } else{ bosa_hotel_breadcrumb(); } ?>
	        </div>
		<?php
		} 
	}
 ...
 */
function bosa_hotel_pingback_header() {
	if ( is_singular() && pings_open() ) {
		printf( '<link rel="pingback" href="%s">', esc_url( get_bloginfo( 'pingback_url' ) ) );
	}
}
add_action( 'wp_head', 'bosa_hotel_pingback_header' );
...
* @return array $class
*/
function bosa_hotel_body_class_modification( $class ){
...
add_filter( 'body_class', 'bosa_hotel_body_class_modification' );

if( !function_exists( 'bosa_hotel_transparent_body_class' ) ){
	/**
	...
	* @return array $class
	*/
	function bosa_hotel_transparent_body_class( $class ){
	...
	add_filter( 'body_class', 'bosa_hotel_transparent_body_class' );
}
 ...
 */
function bosa_hotel_content_width() {
	// This variable is intended to be overruled from themes.
	// Open WPCS issue: {@link https://github.com/WordPress-Coding-Standards/WordPress-Coding-Standards/issues/1043}.
	// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound
	$GLOBALS['content_width'] = apply_filters( 'bosa_hotel_content_width', 720 );
}
add_action( 'after_setup_theme', 'bosa_hotel_content_width', 0 );
...
if( !function_exists( 'bosa_hotel_get_related_posts' ) ):
	function bosa_hotel_get_related_posts( $taxonomy = array(), $per_page = 4, $get_params = false ){
 ...
 * @since Bosa Hotel 1.0.0
 */
function bosa_hotel_widgets_init() {

	register_sidebar( array(
		'name'          => esc_html__( 'Right Sidebar', 'bosa-hotel' ),
		'id'            => 'right-sidebar',
		'description'   => esc_html__( 'Add widgets here.', 'bosa-hotel' ),
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Offcanvas Menu Sidebar', 'bosa-hotel' ),
		'id'            => 'menu-sidebar',
		'description'   => esc_html__( 'Add widgets here.', 'bosa-hotel' ),
		'before_widget' => '<div id="%1$s" class="widget %2$s">',
		'after_widget'  => '</div>',
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Left Sidebar', 'bosa-hotel' ),
		'id'            => 'left-sidebar',
		'description'   => esc_html__( 'Add widgets here.', 'bosa-hotel' ),
		'before_widget' => '<div id="%1$s" class="widget %2$s">',
		'after_widget'  => '</div>',
	...
	register_sidebar( array(
		'name'          => esc_html__( 'WooCommerce Right Sidebar', 'bosa-hotel' ),
	...
	register_sidebar( array(
		'name'          => esc_html__( 'WooCommerce Left Sidebar', 'bosa-hotel' ),
	...
	for( $i = 1; $i <= 4; $i++ ){
		register_sidebar( array(
			'name'          => esc_html__( 'Footer Sidebar', 'bosa-hotel' ) . ' ' . $i,
			'id'            => 'footer-sidebar-' . $i,
			'description'   => esc_html__( 'Add widgets here.', 'bosa-hotel' ),
...
add_action( 'widgets_init', 'bosa_hotel_widgets_init' );
 ...
 */
function bosa_hotel_is_active_footer_sidebar(){
...
if( ! function_exists( 'bosa_hotel_sort_category' ) ):
/**
 * Helper function for bosa_hotel_get_the_category()
 *
 * @since Bosa Hotel 1.0.0
 */
function bosa_hotel_sort_category( $a, $b ){
...
if ( ! function_exists( 'bosa_hotel_validate_excerpt_count' ) ) :
	/**
	 ...
	 */
	function bosa_hotel_validate_excerpt_count( $validity, $value ){
		$value = intval( $value );
		if ( empty( $value ) || ! is_numeric( $value ) ) {
			$validity->add( 'required', esc_html__( 'You must supply a valid number.', 'bosa-hotel' ) );
		} elseif ( $value < 1 ) {
			$validity->add( 'min_slider', esc_html__( 'Minimum no of Excerpt Lenght is 1', 'bosa-hotel' ) );
		} elseif ( $value > 50 ) {
			$validity->add( 'max_slider', esc_html__( 'Maximum no of Excerpt Lenght is 50', 'bosa-hotel' ) );
		}
		return $validity;
	}
endif;
...
function bosa_hotel_modify_archive_title( $title ) {
	if( is_category() ) {
        ...
        $title = get_the_date( _x( 'Y', 'yearly archives date format', 'bosa-hotel' ) );
    } elseif ( is_month() ) {
        $title = get_the_date( _x( 'F Y', 'monthly archives date format', 'bosa-hotel' ) );
    } elseif ( is_day() ) {
        $title = get_the_date( _x( 'F j, Y', 'daily archives date format', 'bosa-hotel' ) );
...
add_filter( 'get_the_archive_title', 'bosa_hotel_modify_archive_title' );

if( ! function_exists( 'bosa_hotel_get_the_category' ) ):
/**
* Returns categories after sorting by term id descending
* 
* @since Bosa Hotel 1.0.0
* @uses bosa_hotel_sort_category()
* @return array
*/
function bosa_hotel_get_the_category( $id = false ){
    $failed = true;
    ...
    if ( class_exists( 'WPSEO_Primary_Term' ) ){
        ...
        $bosa_hotel_cat[0] = get_term( $wpseo_primary_term );

        if ( !is_wp_error( $bosa_hotel_cat[0] ) ) { 
           $failed = false;
        }
    }
    ...
    if( $failed ){
      ...
      $bosa_hotel_cat = get_the_category( $id );
      usort( $bosa_hotel_cat, 'bosa_hotel_sort_category' );  
    }
    
    return $bosa_hotel_cat;
}
...
* 
* @since Bosa Hotel 1.0.0
* @uses bosa_hotel_get_post_categories()
* @return array
*/
function bosa_hotel_get_post_categories(){
...
* @since Bosa Hotel 1.0.0
*/
function bosa_hotel_get_custom_logo_url(){
...
function bosa_hotel_banner(){
	...
	$render_banner_image 	= get_theme_mod( 'render_banner_image_size', 'bosa-hotel-1920-550' );
	$banner_obj 			= wp_get_attachment_image_src( $banner_image_ID, $render_banner_image );
	if( !$banner_image_ID ){
		$banner_image = get_theme_file_uri( '/assets/images/bosa-hotel-1920-550.jpg' );
	}else{
...
* @since Bosa Hotel 1.0.0
*/
function bosa_hotel_footer_image(){
...
* @since Bosa Hotel 1.0.0
*/
function bosa_hotel_get_banner_title(){
...
* @since Bosa Hotel 1.0.0
*/
function bosa_hotel_get_banner_subtitle(){
...
* @since Bosa Hotel 1.0.0
*/
function bosa_hotel_excerpt_length( $length ) {
	if ( is_admin() ) return $length;
	$excerpt_length = get_theme_mod( 'excerpt_length' , 60 );
	return $excerpt_length;	
}
add_filter( 'excerpt_length', 'bosa_hotel_excerpt_length', 999 );

if( !function_exists( 'bosa_hotel_has_social' ) ){
	/**
	...
	* @return bool
	*/
	function bosa_hotel_has_social(){
...
if( !function_exists( 'bosa_hotel_social' ) ){
	/**
	* Add social icons.
	* 
	* @since Bosa Hotel 1.0.0
	*/
	function bosa_hotel_social(){
...
if( !function_exists( 'bosa_hotel_has_header_media' ) ){
	/**
	...
	* @return bool
	*/
	function bosa_hotel_has_header_media(){
...
if( !function_exists( 'bosa_hotel_header_media' ) ){
	/**
	* Add header banner/slider.
	* 
	* @since Bosa Hotel 1.0.0
	*/
	function bosa_hotel_header_media(){
		$header_slider_defaults = array(
			array(
				'slider_item' 	=> '',
			)			
...
* @since Bosa Hotel 1.0.0
*/
function bosa_hotel_post_transparent_banner() { ?>
	<div class="overlay-post inner-banner-wrap">
		<?php
		$feature_image_id 				= get_post_thumbnail_id();
		$render_single_post_image_size 	= get_theme_mod( 'render_single_post_image_size', 'bosa-hotel-1370-550' );
		...
		<div class="inner-banner-content" style="background-image: url( <?php echo esc_url( $transparent_header_inner_banner ); ?> );">
			<div class="container">
				<?php bosa_hotel_single_page_title(); ?>
			</div>
		</div>
		<?php if ( get_theme_mod( 'breadcrumbs_controls', 'show_in_all_page_post' ) == 'disable_in_all_pages' || get_theme_mod( 'breadcrumbs_controls', 'show_in_all_page_post' ) == 'show_in_all_page_post' ){
			bosa_hotel_breadcrumb_wrap( true );
		} ?>
	</div>
<?php }
...
* @since Bosa Hotel 1.0.0
*/
function bosa_hotel_page_transparent_banner() { ?>
	<div class="overlay-page inner-banner-wrap">
		<?php
		$feature_image_id 			= get_post_thumbnail_id();
		$render_pages_image_size 	= get_theme_mod( 'render_pages_image_size', 'bosa-hotel-1370-550' );
...
* @since Bosa Hotel 1.0.0
*/
if( !function_exists( 'bosa_hotel_are_plugin_active' ) ){
	function bosa_hotel_are_plugin_active() {
...
if( !function_exists( 'bosa_hotel_get_intermediate_image_sizes' ) ){
	/**
	...
	* @return array
	*/
	function bosa_hotel_get_intermediate_image_sizes(){

		$data 	= array(
			'full'			=> esc_html__( 'Full Size', 'bosa-hotel' ),
			'large'			=> esc_html__( 'Large Size', 'bosa-hotel' ),
			'medium'		=> esc_html__( 'Medium Size', 'bosa-hotel' ),
			'medium_large'	=> esc_html__( 'Medium Large Size', 'bosa-hotel' ),
			'thumbnail'		=> esc_html__( 'Thumbnail Size', 'bosa-hotel' ),
			'1536x1536'		=> esc_html__( '1536x1536 Size', 'bosa-hotel' ),
			'2048x2048'		=> esc_html__( '2048x2048 Size', 'bosa-hotel' ),
			'bosa-hotel-1920-550' => esc_html__( '1920x550 Size', 'bosa-hotel' ),
			'bosa-hotel-1370-550'	=> esc_html__( '1370x550 Size', 'bosa-hotel' ),
			'bosa-hotel-590-310'	=> esc_html__( '590x310 Size', 'bosa-hotel' ),
			'bosa-hotel-420-380'	=> esc_html__( '420x380 Size', 'bosa-hotel' ),
			'bosa-hotel-420-300'	=> esc_html__( '420x300 Size', 'bosa-hotel' ),
			'bosa-hotel-420-200'	=> esc_html__( '420x200 Size', 'bosa-hotel' ),
			'bosa-hotel-290-150'	=> esc_html__( '290x150 Size', 'bosa-hotel' ),
			'bosa-hotel-80-60'	=> esc_html__( '80x60 Size', 'bosa-hotel' ),
		);
 ...
 * @return array
 */
function bosa_hotel_woocommerce_body_classes( $classes ) {
	
	if( bosa_hotel_wooCom_is_shop() && get_theme_mod( 'disable_sidebar_woocommerce_shop', false ) ){
		$classes[] = 'no-sidebar';
	}
	return $classes;
}
add_filter( 'body_class', 'bosa_hotel_woocommerce_body_classes' );

/**
 * Adds filter to force import demo for advanced import plugin.
 *
 */
function bosa_hotel_prefix_advanced_import_force_proceed() {
	return true;
}

add_action( 'advanced_import_force_proceed', 'bosa_hotel_prefix_advanced_import_force_proceed', 10 );
...
* @return array
*/
function bosa_hotel_get_product_categories(){
...
if( !function_exists( 'bosa_hotel_has_header_buttons' ) ){
    /**
    ...
    * @return bool
    */
    function bosa_hotel_has_header_buttons(){
...
if( !function_exists( 'bosa_hotel_header_buttons' ) ){
    /**
    * Add header buttons.
    * 
    * @since Bosa Hotel 1.0.0
    */
    function bosa_hotel_header_buttons(){
        $header_btn_defaults = array(
            array(
                'header_btn_type'           => 'button-outline',
                'header_btn_bg_color'       => '#C9A76E',
