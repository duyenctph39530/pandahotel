...
if( $bosa_hotel_show_highlight_posts && !get_theme_mod( 'disable_highlight_posts_section', false ) ){
	...
	<section class="section-highlight-post highlight-layout-two">
		<div class="section-highlight-inner">
			<?php if( ( !get_theme_mod( 'disable_highlight_posts_section_title', false ) && get_theme_mod( 'highlight_posts_section_title', 'Highlighted Posts' ) ) || ( !get_theme_mod( 'disable_highlight_posts_section_description', true ) && get_theme_mod( 'highlight_posts_section_description', 'Section Description' ) ) ){ ?>
				<div class="section-title-wrap <?php echo esc_attr( $bosa_hotel_highlight_title_desc_align ); ?>">
					<?php if( !get_theme_mod( 'disable_highlight_posts_section_title', false ) && get_theme_mod( 'highlight_posts_section_title', 'Highlighted Posts' ) ){ ?>
						<h2 class="section-title"><?php echo esc_html( get_theme_mod( 'highlight_posts_section_title', 'Highlighted Posts' ) ); ?></h2>
					<?php }
					if( !get_theme_mod( 'disable_highlight_posts_section_description', true ) && get_theme_mod( 'highlight_posts_section_description', 'Section Description' ) ){ ?>
						<p><?php echo esc_html( get_theme_mod( 'highlight_posts_section_description', 'Section Description' ) ); ?></p>
					<?php } ?>
				</div>
			<?php } ?>
					...
					<div class="slide-item">
						...
						<div class="slide-inner">
							<article id="post-<?php the_ID(); ?>" <?php post_class( $bosa_hotel_noThumbnail ) ?>>
								<?php
					        	$bosa_hotel_highlight_two_size = 'bosa-hotel-420-300';
					        	...
					        	if ( empty( $bosa_hotel_render_highlight_post_image_size ) ){
					        		$bosa_hotel_render_highlight_post_image_size = $bosa_hotel_highlight_two_size;
					        	}
