<?php
$bosa_hotel_posts_per_page_count = get_theme_mod( 'feature_posts_posts_number', 6 );
$bosa_hotel_feature_posts_id = get_theme_mod( 'feature_posts_category', '' );

$bosa_hotel_query = new WP_Query( apply_filters( 'bosa_hotel_blog_args', array(
	'post_type'           => 'post',
	'post_status'         => 'publish',
	'posts_per_page'      => $bosa_hotel_posts_per_page_count,
	'cat'                 => $bosa_hotel_feature_posts_id,
	'offset'              => 0,
	'ignore_sticky_posts' => 1
)));

$bosa_hotel_posts_array = $bosa_hotel_query->get_posts();
$bosa_hotel_show_feature_posts = count( $bosa_hotel_posts_array ) > 0 && is_home();

if( !get_theme_mod( 'disable_feature_posts_section', false ) && $bosa_hotel_show_feature_posts ){
	$bosa_hotel_feature_title_desc_align = get_theme_mod( 'feature_posts_section_title_desc_alignment', 'left' );
	if ( $bosa_hotel_feature_title_desc_align == 'left' ){
		$bosa_hotel_feature_title_desc_align = 'text-left';
	}else if ( $bosa_hotel_feature_title_desc_align == 'center' ){
		$bosa_hotel_feature_title_desc_align = 'text-center';
	}else{
		$bosa_hotel_feature_title_desc_align = 'text-right';
	} ?>
	<section class="section-feature-posts-area feature-posts-layout-one">
		<?php if( ( !get_theme_mod( 'disable_feature_posts_section_title', true ) && get_theme_mod( 'feature_posts_section_title', '' ) ) || ( !get_theme_mod( 'disable_feature_posts_section_description', true ) && get_theme_mod( 'feature_posts_section_description', '' ) ) ){ ?>
			<div class="section-title-wrap <?php echo esc_attr( $bosa_hotel_feature_title_desc_align ); ?> ">
		...
		<div class="content-wrap">
			...
			<?php
				...
				$bosa_hotel_render_feature_post_image_size = get_theme_mod( 'render_feature_post_image_size', 'bosa-hotel-420-300' );
				$bosa_hotel_image 							= get_the_post_thumbnail_url( get_the_ID(), $bosa_hotel_render_feature_post_image_size );

				$bosa_hotel_columns_class = '';
				if( get_theme_mod( 'feature_posts_columns', 'three_columns' ) == 'one_column' ){
					$bosa_hotel_columns_class = 'col-md-12';
				}elseif( get_theme_mod( 'feature_posts_columns', 'three_columns' ) == 'two_columns' ){
					$bosa_hotel_columns_class = 'col-md-6';
				}elseif( get_theme_mod( 'feature_posts_columns', 'three_columns' ) == 'three_columns' ){
					$bosa_hotel_columns_class = 'col-md-4';
				}elseif( get_theme_mod( 'feature_posts_columns', 'three_columns' ) == 'four_columns' ){
					$bosa_hotel_columns_class = 'col-md-3';
				}
				?>
					<div class="<?php echo esc_attr( $bosa_hotel_columns_class ); ?>">
						<article class="post feature-posts-content-wrap <?php echo esc_attr( get_theme_mod( 'feature_posts_text_alignment', 'text-left' ) ); ?>">
							<div class="feature-posts-image" style="background-image: url( <?php echo esc_url( $bosa_hotel_image ); ?> );">
								<div class="feature-posts-content">
									<?php
									if( 'post' == get_post_type() ): 
										$bosa_hotel_categories_list = get_the_category_list( ' ' );
										if( $bosa_hotel_categories_list && !get_theme_mod( 'hide_featured_posts_category', false ) ):
