...
?>
<?php 
if( !get_theme_mod( 'disable_transparent_header_post', true ) && get_theme_mod( 'header_layout', 'header_fourteen' ) == 'header_two' ){
	bosa_hotel_post_transparent_banner();
} ?>
<div id="content" class="site-content">
	<div class="container">
		<section class="wrap-detail-page">
			<?php if( get_theme_mod( 'disable_transparent_header_post', true ) || get_theme_mod( 'header_layout', 'header_fourteen' ) != 'header_two' ){
				if( get_theme_mod( 'post_title_position', 'above_feature_image' ) == 'above_feature_image' ){
				bosa_hotel_single_page_title();
				}		
							...
							if( has_post_thumbnail() ){
								if( get_theme_mod( 'single_feature_image', 'show_in_all_pages' ) == 'show_in_all_pages' ){ ?>
								    <figure class="feature-image single-feature-image">
		    						    <?php 
		    						    $bosa_hotel_render_single_post_image_size 	= get_theme_mod( 'render_single_post_image_size', 'bosa-hotel-1370-550' );
		    						    bosa_hotel_image_size( $bosa_hotel_render_single_post_image_size ); ?>
		    						</figure>
								<?php }else{
			...
			<section class="section-ralated-post">
				<div class="section-title-wrap">
					<h2 class="section-title">
						<?php echo esc_html(get_theme_mod( 'related_posts_title', esc_html__( 'You may also like these', 'bosa-hotel' ) )); ?>
					</h2>
				</div>
