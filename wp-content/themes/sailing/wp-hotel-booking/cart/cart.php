...
<?php if ( $cart->cart_items_count != 0 ) : ?>
	<div id="hotel-booking-cart">
		...
		<form id="hb-cart-form" method="post">
			<h3><?php _e( 'Giỏ hàng', 'sailing' ); ?></h3>
			<table class="hb_table">
				<thead>
				<th>&nbsp;</th>
				<th colspan="2" class="hb_room_type"><?php _e( 'Phòng/Dịch vụ bổ sung', 'sailing' ); ?></th>
				...
				<?php if ( $rooms = $cart->get_rooms() ) : ?>
					<?php foreach ( $rooms as $cart_id => $room ) : ?>
						...
						<tr class="hb_checkout_item" data-cart-id="<?php echo esc_attr( $cart_id ); ?>">
							...
							<td colspan="2" class="hb_room_type">
							...
							<td class="hb_capacity">
								<span><?php echo sprintf( _n( '%d adult', '%d adults', $room->capacity, 'sailing' ), $room->capacity ); ?></span>
								<span><?php echo sprintf( _n( '%d child', '%d childs', $room->max_child, 'sailing' ), $room->max_child ); ?></span>
							</td>
			...
			<p>
				<a href="<?php echo hb_get_checkout_url(); ?>" class="hb_button hb_checkout"><?php _e( 'Thanh toán', 'sailing' ); ?></a>
			</p>
		</form>
	</div>
