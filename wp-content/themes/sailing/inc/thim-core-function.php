	...
	function thim_add_metabox_settings( $meta_box, $prefix ) {
		...
		if (class_exists('\Thim_EL_Kit\Functions') && isset($_GET['post']) && (get_option('woocommerce_cart_page_id') == $_GET['post'])) {
			$meta_box['tabs']['product_setting'] = array(
				'label' => __('Product Setting', 'sailing'),
				'icon'  => 'dashicons-editor-insertmore',
			);
			$loop_product_item                   = array(
				'name'    => esc_html__('Select Loop Item', 'sailing'),
				'id'      => 'thim_loop_item_content_product',
				'type'    => 'select',
				'options' => array('' => esc_html__('Default', 'sailing')) + Functions::instance()->get_pages_loop_item('product'),
				'tab'     => 'product_setting',
			);
		}
...
if ( ! function_exists( 'thim_import_add_page_id_settings' ) ) {
	function thim_import_add_page_id_settings( $settings ) {
		...
		$settings[] = 'tp_hotel_booking_rooms_page_id';
...
add_filter('thim_importer_basic_settings', 'thim_extra_import_tp_sailing_settings');
function thim_extra_import_tp_sailing_settings($settings){
	$settings[] =  'hotel_booking_update_meta_field_rooms';
	...
	$settings[]	=  'tp_hotel_booking_enable_advanced_review';
