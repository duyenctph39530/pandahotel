...
if ( ! function_exists( 'hb_meta_box_field_coupon_date' ) ) {
	...
	function hb_meta_box_field_coupon_date( $value ) {
		// Kiểm tra xem giá trị có phải là timestamp hay không
		if ( is_numeric( $value ) && intval( $value ) ) {
			// Nếu là timestamp, trả về ngày định dạng
			return date( hb_get_date_format(), intval( $value ) );
		} else {
			// Nếu không phải timestamp, cố gắng chuyển đổi chuỗi ngày thành timestamp
			$timestamp = strtotime( $value ); // Chuyển chuỗi ngày thành timestamp
			if ( $timestamp ) {
				// Trả về ngày đã được định dạng
				return date( hb_get_date_format(), $timestamp );
			}
		}
